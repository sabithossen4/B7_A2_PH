...
Not deployed yet. Replace this line with the public Render/Railway URL after deployment.
...
- Node.js 24 or newer
- A PostgreSQL database (Neon, Supabase, Railway, or local PostgreSQL)
...
Run strict type checking and a production build:
...
The integration smoke test uses the configured database, creates uniquely named temporary users and an issue, verifies authentication and permissions, and deletes those records when finished:
...
A `render.yaml` Blueprint is included. On Render, connect the repository, provide `CONNECTIONSTRING`, and deploy. `JWT_SECRET` is generated by Render. Set `CORS_ORIGIN` to the frontend URL if a frontend is added.

For Railway, use `npm run build` as the build command and `npm start` as the start command, then configure the same environment variables.
